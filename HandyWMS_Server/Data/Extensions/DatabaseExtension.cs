...
namespace HandyWMS_Server.Data.Extensions
{
    public static class DatabasesExtension
    {
        ...
        public static List<T> IDataReaderToList<T>(IDataReader reader)
        {
            using (reader)
            {
                ...
                while (reader.Read())
                {
                    ...
                    foreach (PropertyInfo property in ReflectionUtil.GetProperties(model.GetType()))
                    {
        ...
        public static Hashtable GetPropertyInfo<T>(T entity)
        {
            ...
            PropertyInfo[] props = ReflectionUtil.GetProperties(entity.GetType());
        ...
        public static IQueryable<T> AppendSort<T>(IQueryable<T> tempData, string sort, bool isAsc)
        {
            ...
            for (int index = 0; index < sortArr.Length; index++)
            {
                ...
                var property = ReflectionUtil.GetProperties(typeof(T)).Where(p => p.Name.ToLower() == sortField.ToLower()).FirstOrDefault();
