...
namespace HandyWMS_ClassLibrary.Utils
{
    public class CaptchaHelper
    {
        ...
        public static byte[] CreateCaptchaImage(string randomCode)
        {
            ...
            int mapHeight = 28;
            Random random = new Random();
            using (SKBitmap bitmap = new SKBitmap(mapwidth, mapHeight))
            {
                using (SKCanvas canvas = new SKCanvas(bitmap))
                {
                    ...
                    using (SKPaint paint = new SKPaint())
                    {
                        ...
                        canvas.DrawCircle(mapwidth / 2, mapHeight / 2, 300, paint);
                    }
                    ...
                    using (SKPaint noisePaint = new SKPaint())
                    {
                        ...
                        {
                            int x1 = random.Next(mapwidth);
                            int y1 = random.Next(mapHeight);
                            canvas.DrawPoint(x1, y1, noisePaint);
                        }
                    }

                    using (SKFileWStream outStream = new SKFileWStream("myImage.png"))
                    {
                        SKPixmapEncoder.Encode(outStream, bitmap, SKEncodedImageFormat.Png, 100);
                    }
                }
            }
